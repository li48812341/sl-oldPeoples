...
import { getPersonAbnormalEntryList, getCurrentVideoUrl } from '@/lib/service/common'
...
const User = () => {
  ...
  // const { refreshList2, setRefreshAbnormityList } = useStore(); // 刷新异常列表
  ...
  const fetchData = async (params: any) => {
    try {
      const res:any = await getPersonAbnormalEntryList(params);
  ...
  // useEffect(() => {
  //   console.log("刷新列表",refreshList2)
  //   if (refreshList2) {
  //     fetchData({ pageNo: 1, pageSize: 10 }); // 收到 WebSocket 消息后重新获取数据
  //     setRefreshAbnormityList(false); // 重置刷新状态
  //   }
  // }, [refreshList2, setRefreshAbnormityList]);
